 ...
 <v-container fluid>
  <v-row>
    <schooladmin-navigation></schooladmin-navigation>
      ...
      <!-- announcements title field -->
                    <v-card-text class="px-5">
                      <v-text-field
                      :rules="[v => !!v || 'Field is required']"
                        label="Title"
                        v-model="announcements_items.title"
                        prepend-inner-icon="mdi-pencil"
                      ></v-text-field>
      
                    </v-card-text>
  
                    <!-- announcements body field -->
                    <v-card-text class="px-5">
                      <v-textarea
                      :rules="[v => !!v || 'Field is required']"
                        label="Body"
                        v-model="announcements_items.body"
                        prepend-inner-icon="mdi-pencil"
                      ></v-textarea>
                    </v-card-text>
                      <!-- author ID field -->
                    <v-card-text class="px-5">
                      ...
                      :rules="[v => !!v || 'Field is required']"
                        label="author id"
                        v-model="announcements_items.author_id"
                        prepend-inner-icon="mdi-pencil"
                      ></v-text-field>
    ...
    export default {
      ...
      data: () => ({
          ...
          headers: [
           ...
           { text: "Author ID", value: "author_id" },
        ...
        methods: {
          ...
          async AnnouncementList() {
            var res = await axios.get("/announcements");
            if (res.status === 200) this.announcements = res.data;
            else {
              this.$Notice.error({
                title: "Error",
          ...
          editItem(item) {
            ...
            this.announcements_items = Object.assign({}, item);
            this.dialog = true;
            // console.log(item);
          },
          deleteItem(item) {
          ...
          async deleteItemConfirm() {
            var res = await axios.delete("/announcements/delete/" + this.id);
            if (res.status === 200) {
              this.AnnouncementList();
          ...
          close() {
            this.dialog = false;
            this.$nextTick(() => {
              this.announcements_items = Object.assign({}, this.defaultItem);
              this.editedIndex = -1;
            });
          },
          closeDelete() {
          ...
          Submit() {
            if (this.$refs.form.validate()) {
              if (this.id) {
                axios
                  .put("/announcements/update/" + this.id, this.announcements_items)
                  .then((response) => {
                    this.AnnouncementList();
                    this.close();
                  })
                ...
                axios
                  .post("/announcements/create", this.announcements_items)
                  .then((response) => {
                    this.AnnouncementList();
                    this.close();
