      ...
      <!-- modal for edit -->
                ...
                <!--  subject name field -->
                    <v-card-text class="px-5">
                      <v-text-field
                        label="subject name"
                        v-model="subjects_items.subject_name"
      ...
      data: () => ({
        ...
        methods: {
          ...
          async SubjectList() {
            ...
            if (res.status === 200) this.subjects = res.data;
            else {
              this.$Notice.error({
                title: "Error",
          ...
          editItem(item) {
            ...
            this.subjects_items = Object.assign({}, item);
            this.dialog = true;
            // console.log(item);
          },
          deleteItem(item) {
          ...
          async deleteItemConfirm() {
            var res = await axios.delete("/subjects/delete/" + this.id);
            if (res.status === 200) {
              this.SubjectList();
          ...
          close() {
            this.dialog = false;
            this.$nextTick(() => {
              this.subjects_items = Object.assign({}, this.defaultItem);
              this.editedIndex = -1;
            });
          },
          closeDelete() {
          ...
          Submit() {
            if (this.$refs.form.validate()) {
              if (this.id) {
                axios
                  .put("/subjects/update/" + this.id, this.subjects_items)
                  .then((response) => {
                    this.SubjectList();
                    this.close();
                  })
                ...
                axios
                  .post("/subjects/create", this.subjects_items)
                  .then((response) => {
                    this.SubjectList();
                    this.close();
                  })
