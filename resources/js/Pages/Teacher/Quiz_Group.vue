                ...
                <!--  title field -->
                    <v-card-text class="px-5">
                     ...
                     <v-card-text class="px-5">
                      <v-text-field
                        label="description"
                        v-model="quizzes_items.description"
                        prepend-inner-icon="mdi-pencil"
                      ></v-text-field>
                    </v-card-text>
    ...
    export default {
      ...
      data: () => ({
        ...
        methods: {
          ...
          async QuizList() {
            var res = await axios.get("/quizzes");
            if (res.status === 200) this.quizzes = res.data;
            else {
              this.$Notice.error({
                title: "Error",
          ...
          editItem(item) {
            ...
            this.quizzes_items = Object.assign({}, item);
            this.dialog = true;
            // console.log(item);
          },
          deleteItem(item) {
          ...
          async deleteItemConfirm() {
            var res = await axios.delete("/quizzes/delete/" + this.id);
            if (res.status === 200) {
              this.QuizList();
          ...
          close() {
            this.dialog = false;
            this.$nextTick(() => {
              this.quizzes_items = Object.assign({}, this.defaultItem);
              this.editedIndex = -1;
            });
          },
          closeDelete() {
          ...
          Submit() {
            if (this.$refs.form.validate()) {
              if (this.id) {
                axios
                  .put("/quizzes/update/" + this.id, this.quizzes_items)
                  .then((response) => {
                    this.QuizList();
                    this.close();
                  })
                ...
                axios
                  .post("/quizzes/create", this.quizzes_items)
                  .then((response) => {
                    this.QuizList();
                    this.close();
                  })
