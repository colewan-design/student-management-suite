                ...
                <!--  title field -->
                    <v-card-text class="px-5">
                     ...
                     <v-card-text class="px-5">
                      <v-text-field
                        label="description"
                        v-model="exams_items.description"
                        prepend-inner-icon="mdi-pencil"
                      ></v-text-field>
                    </v-card-text>
    ...
    export default {
      ...
      data: () => ({
        ...
        methods: {
          ...
          async ExamList() {
            var res = await axios.get("/exams");
            if (res.status === 200) this.exams = res.data;
            else {
              this.$Notice.error({
                title: "Error",
          ...
          editItem(item) {
            ...
            this.exams_items = Object.assign({}, item);
            this.dialog = true;
            // console.log(item);
          },
          deleteItem(item) {
          ...
          async deleteItemConfirm() {
            var res = await axios.delete("/exams/delete/" + this.id);
            if (res.status === 200) {
              this.ExamList();
          ...
          close() {
            this.dialog = false;
            this.$nextTick(() => {
              this.exams_items = Object.assign({}, this.defaultItem);
              this.editedIndex = -1;
            });
          },
          closeDelete() {
          ...
          Submit() {
            if (this.$refs.form.validate()) {
              if (this.id) {
                axios
                  .put("/exams/update/" + this.id, this.exams_items)
                  .then((response) => {
                    this.ExamList();
                    this.close();
                  })
                ...
                axios
                  .post("/exams/create", this.exams_items)
                  .then((response) => {
                    this.ExamList();
                    this.close();
                  })
